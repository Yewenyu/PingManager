...
-(void)startPinging {
    if (self.isReady && !self.isPinging) {
        ...
        self.isPinging = YES;
        if (self.isListenThread && self.listenThread == nil){
            self.listenThread = [[NSThread alloc] initWithTarget:self selector:@selector(listenLoop) object:nil];
        ...
        if(self.isSendThread && self.sendThread == nil){
            self.sendThread = [[NSThread alloc] initWithTarget:self selector:@selector(sendLoop) object:nil];
...
-(void)sendPing {
    if (self.isPinging) {
        ...
        sa_family_t hostAddressFamily = self.hostAddressFamily;
...
//        int count = CFGetRetainCount((__bridge CFTypeRef)(self));
        self.setupQueue = dispatch_queue_create("GBPing setup queue", 0);
        self.isStopped = YES;
        self.identifier = arc4random();
        self.isSendThread = YES;
        self.isListenThread = YES;
    }
